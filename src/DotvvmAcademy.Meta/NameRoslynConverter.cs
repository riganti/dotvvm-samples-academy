...
{
    public class NameRoslynConverter : IMetaConverter<NameNode, ISymbol>
    {
        private readonly IMetaContext context;

        public NameRoslynConverter(IMetaContext context)
        {
            this.context = context;
        }

        public IEnumerable<ISymbol> Convert(NameNode source)
        {
            return Visit(source);
        }
        ...
        {
            yield return context.Compilation.GlobalNamespace;
        ...
        private IEnumerable<ISymbol> VisitArrayType(ArrayTypeNameNode arrayType)
        {
            ...
            return Visit(arrayType.ElementType)
                .OfType<ITypeSymbol>()
                .Select(t => context.Compilation.CreateArrayTypeSymbol(t, rank));
        }
        ...
        private IEnumerable<ISymbol> VisitPointerType(PointerTypeNameNode pointerType)
        {
            return Visit(pointerType.ElementType)
                .Cast<ITypeSymbol>()
                .Select(t => context.Compilation.CreatePointerTypeSymbol(t));
        }
        ...
        private IEnumerable<ISymbol> VisitSimple(INamespaceSymbol namespaceSymbol, SimpleNameNode simple)
        {
            switch (simple)
            {
                ...
                default:
                    throw new NotSupportedException($"{nameof(SimpleNameNode)} type '{simple.GetType()}' is not supported.");
            }
        }
    }
}
